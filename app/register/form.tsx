...
import { User, Mail, Lock, Ticket, Loader2 } from "lucide-react";
...
export default function RegisterForm() {
  ...
  useEffect(() => {
    if (errorMessage === "SUCCESS") {
      // Redirigir a login con query param indicando registro exitoso
  ...
  return (
    <div className="flex min-h-screen w-full items-center justify-center px-4 bg-[#f8fafc] relative overflow-hidden">
      ...
      <Card className="relative z-10 w-full max-w-md bg-white border border-[#c6c6cd] shadow-lg rounded-2xl overflow-hidden py-4">
        <CardHeader className="text-center flex flex-col items-center pb-2">
          <div className="p-3 bg-[#0051d5]/10 text-[#0051d5] rounded-2xl mb-3 border border-[#0051d5]/20">
            <Ticket size={32} />
          </div>
          <CardTitle className="text-2xl font-bold text-[#131b2e] tracking-tight">Registro de Cuenta</CardTitle>
          <CardDescription className="text-gray-500 mt-1">
            Crea tu cuenta institucional de soporte técnico
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <form action={dispatch} className="space-y-4">
            ...
            {errorMessage && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-xs text-[#ba1a1a] font-medium leading-relaxed">
                {errorMessage}
              </div>
            )}
        ...
        <CardFooter className="pt-2 pb-0 flex flex-col gap-2">
          <div className="text-xs text-center w-full text-gray-500">
            ...
            <Link href="/login" className="text-[#0051d5] font-semibold hover:underline transition-colors">
              Inicia sesión
            </Link>
          </div>
        </CardFooter>
